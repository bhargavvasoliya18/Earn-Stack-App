...
class ArticleHelper {
  ...
  Future<List<ArticleModel>> getArticle(context,
      {int page = 1, bool showLoader = false, List<String>? completeQuizs, List<String>? completeArticles}) async {
    List<ArticleModel> tempArticleList = [];
    String authToken = await sharedPref.read("authToken");
    try {
      var res = await ApiService.request(context, "${AppUrls.getArticle}?per_page_data=${10}&page=$page", RequestMethods.GET,
  ...
  readArticleAndUpdateTime(context, time) async {
    Map<String, dynamic> body = {"user_id": loginResponseModel.id, "time": time};
    String authToken = await sharedPref.read("authToken");
    try {
  ...
  quizAndReadArticleComplete(context, String postId, {bool? isReadArticle}) async {
    String authToken = await sharedPref.read("authToken");
    Map<String, dynamic> body = {"user_id": loginResponseModel.id, "post_id": postId, "is_read": 1};
    try {
      var res = await ApiService.request(
  ...
  Future<List<String>> getReadArticleAndPlayQuiz(context, String type) async {
    ...
    String authToken = await sharedPref.read("authToken");
    Map<String, dynamic> body = {"user_id": loginResponseModel.id, "type": type};
    try {
